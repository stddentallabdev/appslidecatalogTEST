...
const generateCatalog = (): Record<string, CatalogItem> => {
  ...
  for (let i = 1; i <= 40; i++) {
    ...
    catalog[id] = {
      ...
      description: desc,
      videoSrc: `/videos/${id}.mp4`, // Expects stdr01.mp4, stdr02.mp4 etc. in public folder
      badge: badge,
...
VIDEO_CATALOG["rtn2026"] = {
  id: "rtn2026",
  name: "แคตตาล็อกรวม รีเทนเนอร์ 2026",
  description: "วิดีโอรวมผลิตภัณฑ์รีเทนเนอร์ทุกคอลเลกชันปี 2026 จาก S.T.D. DENTAL LAB คุณภาพเกรดการแพทย์ระดับพรีเมียม ให้ภาพคมชัดละเอียด ประณีต ทุกมิติการสแกน",
  videoSrc: "/videos/rtn2026.mp4",
  badge: "New Catalog 2026",
  likes: 2480,
  views: 15420
};
...
export default function Home() {
  ...
  // 1. IG Reels Style Auto-Play Next on Ended Event
  const handleVideoEnded = () => {
    if (!isAutoPlayNext) return;

    // Increment index to the next product
  ...
  // 2. Navigation Helper that dynamically syncs standard browser URLs
  const navigateToItem = (id: string) => {
    ...
    // Smooth URL updating without full page reload (highly professional)
    ...
    <main className="relative flex flex-1 h-screen w-screen bg-slate-950 overflow-hidden select-none font-sans text-white">
      ...
      >
        <video
          ...
          ref={videoRef}
          src={videoSource}
          loop={!isAutoPlayNext} // Loop if autoNext is off, otherwise go to next
          autoPlay
          muted={isMuted}
          playsInline
      ...
      <div className="absolute bottom-0 left-0 w-full sm:w-[75%] z-20 p-5 sm:p-8 flex flex-col gap-3.5 text-white pointer-events-none">
        ...
        {/* Dynamic customized descriptions */}
      ...
      <div className="absolute bottom-0 right-0 z-30 p-5 sm:p-8 flex flex-col items-center gap-6 pointer-events-auto">
        ...
        {/* A. Dynamic Bouncing "Tap to Unmute" floating overlay indicator */}
        <AnimatePresence>
          {showUnmutePrompt && (
            <motion.div
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <span className="text-[9px] font-extrabold tracking-wider drop-shadow-md">แชร์คลิป</span>
        </div>

        {/* D. Auto-Next Toggle (Reels-loop vs Next-autoplay switcher) */}
        <div className="flex flex-col items-center gap-1">
          <button
            onClick={(e) => {
              e.stopPropagation();
        ...
        {/* F. Glowing LINE CTA Button (Top-tier conversion point) */}
        <div className="flex flex-col items-center gap-1 mt-2">
          <a
            href="https://line.me/R/ti/p/@std-dentallab"
            target="_blank"
            rel="noopener noreferrer"
            onClick={(e) => e.stopPropagation()} // Avoid triggering pause
      ...
      {/* Onscreen direct mute warning helper overlay */}
      <AnimatePresence>
        {showUnmutePrompt && (
          <motion.div
            initial={{ opacity: 0 }}
